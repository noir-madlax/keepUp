...
请将以下内容翻译成中文："""
    ...
    async def get_summary(content: str, request_id: int, article_id: int, lang: str) -> Optional[str]:
        ...
        if 'full_content' in processed_response:
            additional_content = BedrockService.extract_additional_processing_content(processed_response['full_content'])
            if additional_content:
                await BedrockService.save_additional_processing_result(article_id, additional_content, lang)
